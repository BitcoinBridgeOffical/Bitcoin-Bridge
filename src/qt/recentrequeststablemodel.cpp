    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return BitcoinBridgeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, BitcoinBridgeUnits::separatorNever);
            else
                return BitcoinBridgeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " (" + BitcoinBridgeUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
