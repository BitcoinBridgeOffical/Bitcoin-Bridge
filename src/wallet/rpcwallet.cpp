...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new BitcoinBridge address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new bitcoinbridge address\n"
            "\nExamples:\n" +
    ...
    return CBitcoinBridgeAddress(keyID).ToString();
}


CBitcoinBridgeAddress GetAccountAddress(CWallet* const pwallet, std::string strAccount, bool bForceNew = false)
{
    CPubKey pubKey;
    ...
    return CBitcoinBridgeAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current BitcoinBridge address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account bitcoinbridge address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 0)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new BitcoinBridge address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinbridge address to be associated with an account.\n"
    ...
    CBitcoinBridgeAddress address(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinBridge address");
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinbridge address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a bitcoinbridge address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
    ...
    for (const std::pair<CBitcoinBridgeAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        const CBitcoinBridgeAddress& address = item.first;
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The bitcoinbridge address to send to.\n"
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            ...
                            "                             The recipient will receive less bitcoinbridges than you enter in the amount field.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The bitcoinbridge address\n"
    ...
    for (std::set<CTxDestination> grouping : pwallet->GetAddressGroupings()) {
        ...
        for (CTxDestination address : grouping) {
            ...
            addressInfo.push_back(CBitcoinBridgeAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwallet->mapAddressBook.find(CBitcoinBridgeAddress(address).Get()) != pwallet->mapAddressBook.end()) {
                    addressInfo.push_back(pwallet->mapAddressBook.find(CBitcoinBridgeAddress(address).Get())->second.name);
                }
            }
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               "\nArguments:\n"
                                               "1. \"address\"         (string, required) The bitcoinbridge address to use for the private key.\n"
    ...
    CBitcoinBridgeAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinbridge address for transactions.\n"
    ...
    CBitcoinBridgeAddress address = CBitcoinBridgeAddress(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinBridge address");
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a bitcoinbridge address." +
            HelpRequiringPassphrase(pwallet) + "\n"
                                               "\nArguments:\n"
                                               ...
                                               "                       the spend.\n"
                                               "2. \"toaddress\"         (string, required) The bitcoinbridge address to send funds to.\n"
    ...
    std::string strAccount = AccountFromValue(request.params[0]);
    CBitcoinBridgeAddress address(request.params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinBridge address");
    CAmount nAmount = AmountFromValue(request.params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               ...
                                               "    {\n"
                                               "      \"address\":amount   (numeric or string) The bitcoinbridge address is the key, the numeric amount (can be string) in " +
            CURRENCY_UNIT + " is the value\n"
                            ...
                            "                           Those recipients will receive less bitcoinbridges than you enter in their corresponding amount field.\n"
    ...
    std::set<CBitcoinBridgeAddress> setAddress;
    std::vector<CRecipient> vecSend;

    CAmount totalAmount = 0;
    std::vector<std::string> keys = sendTo.getKeys();
    for (const std::string& name_ : keys) {
        CBitcoinBridgeAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BitcoinBridge address: ") + name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                          ...
                          "Each key is a BitcoinBridge address or hex-encoded public key.\n"
                          "If 'account' is specified (DEPRECATED), assign address to that account.\n"

                          "\nArguments:\n"
                          "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                          "2. \"keys\"         (string, required) A json array of bitcoinbridge addresses or hex-encoded public keys\n"
                          "     [\n"
                          "       \"address\"  (string) bitcoinbridge address or hex-encoded public key\n"
                          "       ...,\n"
                          "     ]\n"
                          "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

                          "\nResult:\n"
                          "\"address\"         (string) A bitcoinbridge address associated with the keys.\n"
    ...
    return CBitcoinBridgeAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    return CBitcoinBridgeAddress(w.result).ToString();
}
...
UniValue ListReceived(CWallet* const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map<CBitcoinBridgeAddress, tallyitem> mapTally;
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        const CWalletTx& wtx = pairWtx.second;
    ...
    for (const std::pair<CBitcoinBridgeAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        ...
        std::map<CBitcoinBridgeAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The bitcoinbridge address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The bitcoinbridge address of the transaction. Not present for move transactions (category = move).\n"
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + ". This is negative and only available for the \n"
                            ...
                            "      \"address\" : \"address\",          (string) The bitcoinbridge address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending bitcoinbridges\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending bitcoinbridge\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
    ...
    StartShutdown();
    return "wallet encrypted; BitcoinBridge server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending bitcoinbridges.\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of bitcoinbridge addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) bitcoinbridge address\n"
            "      ,...\n"
            "    ]\n"
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "    \"vout\" : n,               (numeric) the vout value\n"
                            "    \"address\" : \"address\",    (string) the bitcoinbridge address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CBitcoinBridgeAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BitcoinBridge address: ") + input.get_str());
            if (setAddress.count(address))
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CBitcoinBridgeAddress(address).ToString()));
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "   {\n"
            "     \"changeAddress\"          (string, optional, default pool address) The bitcoinbridge address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
        if (request.params[1].type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress")) {
                CBitcoinBridgeAddress address(options["changeAddress"].get_str());

                if (!address.IsValid())
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid bitcoinbridge address");
