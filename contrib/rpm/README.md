...
The RPM spec file provided here is for BitcoinBridge-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinbridge.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinBridge git repository](https://github.com/bitcoinbridge/bitcoinbridge) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinbridge.conf` file.
...
in the bitcoinbridge git repository. At present only an SVG version of the BitcoinBridge
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinbridge
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinBridge git
master branch.
...
idea to build BitcoinBridge Core against a static version of that library compiled
...
The `bitcoinbridge.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinbridge-server` package. At this point, most RPM based
...
The `bitcoinbridge-server` package should have SELinux support. How to properly do
...
BitcoinBridge-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinbridge/). He has been
packaging BitcoinBridge for Fedora at least since 2012.
...
4. I renamed the `bitcoinbridge` package that contains the Qt GUI to `bitcoinbridge-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinbridge-xt` or `bitcoinbridge-classic`. I wanted to make sure the general population
