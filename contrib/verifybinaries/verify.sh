...
###   bitcoinbridgecore.org and bitcoinbridge.org and compares them.
...
WORKINGDIR="/tmp/bitcoinbridge_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinbridgecore.org"
HOST2="https://bitcoinbridge.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinbridge-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinbridge-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinbridge.org failed to provide signature file, but bitcoinbridgecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinbridge.org and bitcoinbridgecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinBridge Core binary release signing key installed?"
   fi
