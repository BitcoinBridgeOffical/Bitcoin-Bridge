BitcoinBridge Core version 0.11.0 is now available from:

  <https://bitcoinbridge.org/bin/bitcoinbridge-core-0.11.0/>
...
For example, add the following to `bitcoinbridge.conf`:
...
related to the blockchain in the bitcoinbridge system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BitcoinBridge Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BitcoinBridge Core will 
...
For an example script see [bitcoinbridge-submittx](https://github.com/laanwj/bitcoinbridge-submittx).
...
- #5334 `cf87536` libbitcoinbridgeconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitcoinbridge-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BitcoinBridgeAmountField
- #5753 `0f44672` Add bitcoinbridge logo to about screen
...
- #5809 `46bfbe7` Add bitcoinbridge-cli man page
...
- #5849 `41259ca` contrib/init/bitcoinbridged.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BitcoinBridge-Qt.app to BitcoinBridge-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoinbridge-tx)
